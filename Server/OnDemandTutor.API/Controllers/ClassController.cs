...
public class ClassController : ControllerBase
{
    ...
    public ClassController(IClassServices classServices, IAuthServices authServices,
    ...
    {
        if (id != classDto.Id) return BadRequest("ID mismatch between route parameter and request body.");
        var updatedClass = await _classServices.UpdateClassAsync(classDto);
        if (updatedClass == null) return NotFound();
        return NoContent();
    }
    ...
    public async Task<IActionResult> RatingClass([FromBody] AddRatingDto request)
    {
        var student = await _authServices.GetUserProfileByClaim(HttpContext.User);
        var result =
            await _studentClassService.StudentRatingClassAsync(request.ClassId, student.Id, request.Rating,
                request.Feedback);
        return Ok(result);
    }
}