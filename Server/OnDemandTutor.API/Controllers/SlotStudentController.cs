...
public class SlotStudentController : ControllerBase
{
    ...
    public async Task<IActionResult> QuerySlotStudentOfTutor(
        [FromQuery] PagingModel<QueryRatingDto> querySlotStudentDto)
    {
    ...
    public async Task<IActionResult> GetStudentSlotsOfSlot([FromRoute] int slotId, [FromQuery] int page,
        [FromQuery] int limit)
    {
    ...
    public async Task<IActionResult> GetSlotStudent(int slotId, int studentId)
    {
        ...
        if (slotStudent == null) return NotFound();
        return Ok(slotStudent);
    }
    ...
    [ProducesResponseType(typeof(ApiErrorActionResult), 400)]
    [ProducesResponseType(204)]
    public async Task<IActionResult> UpdateSlotStudent([FromQuery] int slotId,
        [FromBody] UpdateSlotStudentDto updateDto)
    {
        var user = await _authServices.GetUserProfileByClaim(HttpContext.User);
        var result =
            await _slotStudentService.UpdateSlotStudentAsync(slotId, user.Id, updateDto.Rate, updateDto.Feedback);
