...
public class TutorSubjectController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateTutorSubject([FromBody] CreateTutorSubjectDto tutorSubjectDto)
    {
        ...
        return CreatedAtAction(nameof(GetTutorSubjectById), new { createdTutorSubject.Id }, createdTutorSubject);
    }
    ...
    {
        if (id != tutorSubjectDto.Id) return BadRequest("ID mismatch between route parameter and request body.");
        var updatedTutorSubject = await _tutorSubjectService.UpdateTutorSubjectAsync(tutorSubjectDto);
        if (updatedTutorSubject == null) return NotFound();
        return NoContent();
    }
