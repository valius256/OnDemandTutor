...
public class SlotStudentRepository : GenericRepository<SlotStudent>, ISlotStudentRepository
{
    ...
    public async Task<List<SlotStudent>> GetStudentSlotsAsync(QuerySlotStudentDto request, int? studentId)
    {
        ...
        if (request.PaymentStatus.HasValue) query = query.Where(s => s.PaymentStatus == request.PaymentStatus);
        if (request.ClassId.HasValue) query = query.Where(s => s.Slot.ClassId == request.ClassId);
        if (studentId.HasValue) query = query.Where(s => s.UserId == studentId);
        if (request.TutorId.HasValue) query = query.Where(s => s.Slot.CreateById == request.TutorId);
    ...
    public async Task<PagedResult<SlotStudent>> GetStudentSlotByTutor(PagingModel<QueryRatingDto> queryDto)
    {
        ...
        {
            if (queryDto.Filter.TutorId != 0) query = query.Where(s => s.Slot.CreateById == queryDto.Filter.TutorId);
        ...
        return await query.ToNewPagingAsync(queryDto.Page > 0 ? queryDto.Page : 1,
            queryDto.Limit > 0 ? queryDto.Limit : 10);
    }
    ...
    public async Task<List<SlotStudent>> GetAboutToStartStudentSlots()
    {
        var query = dbSet.AsQueryable()
            .Include(ss => ss.Slot)
            .Include(ss => ss.User)
            .Where(ss => ss.Slot.StartTime >= DateTime.Now.AddHours(-1) && ss.PaymentStatus == PaymentStatus.Notpaid);
