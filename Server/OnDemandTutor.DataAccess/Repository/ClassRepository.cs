        ...
        if (pagingModel.Filter != null)
        {
            ...
            if (pagingModel.Filter.StartTime.HasValue)
                classQuery = classQuery.Where(c =>
                    c.Slots.OrderBy(s => s.StartTime).First().StartTime >= pagingModel.Filter.StartTime.Value);

            if (pagingModel.Filter.EndTime.HasValue)
                classQuery = classQuery.Where(c =>
                    c.Slots.OrderBy(s => s.StartTime).Last().EndTime <= pagingModel.Filter.EndTime.Value);
            ...
            if (!string.IsNullOrWhiteSpace(pagingModel.Filter.UserName))
                classQuery = classQuery.Where(c =>
                    (c.Tutor.FirstName + " " + c.Tutor.LastName).Contains(pagingModel.Filter.UserName));
        }
