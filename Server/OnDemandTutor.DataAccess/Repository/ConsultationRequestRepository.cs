...
public class ConsultationRequestRepository : GenericRepository<ConsultationRequest>, IConsultationRequestRepository
{
    ...
    public async Task<PagedResult<ConsultationRequest>> ViewAllConsultationsRequestAsync(
        ConsultationRequestFilterDto request)
    {
        ...
        if (!string.IsNullOrEmpty(request.ConsultationContent))
            consultListQuery =
                consultListQuery.Where(cs => cs.ConsultationContent.Contains(request.ConsultationContent));
