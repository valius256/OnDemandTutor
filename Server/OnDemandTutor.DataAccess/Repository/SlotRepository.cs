...
public class SlotRepository : GenericRepository<Slot>, ISlotRepository
{
    ...
    public async Task<PagedResult<GetSlotsDtos>> GetSlotsAsync(PagingModel<QuerySlotDto> request)
    {
        ...
        {
            if (request.Filter.ClassId.HasValue) query = query.Where(s => s.ClassId == request.Filter.ClassId);
            if (request.Filter.UserId.HasValue) query = query.Where(s => s.CreateById == request.Filter.UserId);
            if (request.Filter.SubjectId.HasValue) query = query.Where(s => s.SubjectId == request.Filter.SubjectId);
            if (request.Filter.Start.HasValue) query = query.Where(s => s.StartTime >= request.Filter.Start);
            if (request.Filter.End.HasValue) query = query.Where(s => s.EndTime <= request.Filter.End);
        }
