...
public class SlotStudentService : ISlotStudentServices
{
    ...
    public SlotStudentService(IUnitOfWorkRepository unitOfWorkRepository, INotificationService notificationService,
        IUserServices userServices, ITransactionServices transactionServices)
    {
    ...
    public async Task<List<GetSlotStudentDetailDto>> QuerySlotStudent(QuerySlotStudentDto querySlotStudentDto,
    ...
    {
        return (await _unitOfWorkRepository.SlotStudentRepository.GetSlotOfStudent(studentId))
            .Adapt<List<GetSlotStudentDetailDto>>();
    }

    public async Task<PagedResult<GetSlotStudentDetailDto>> GetStudentSlotByTutor(
        PagingModel<QueryRatingDto> queryRatingDto)
    {
        var slotStudent =
    ...
    public async Task<PagedResult<GetSlotStudentWithDetailStudentDto>> GetSlotStudentsOfSlotPaged(int slotId, int page,
        int limit)
    {
        var slotStudents =
            await _unitOfWorkRepository.SlotStudentRepository.GetStudentsSlotWithStudentBySlotIdPaged(slotId, page,
                limit);
    ...
    {
        var slotStudentModel = await _unitOfWorkRepository.SlotStudentRepository.Where(ss => ss.PaymentStatus == status)
            .ToListAsync();
    ...
    {
        var slotstudent =
            await _unitOfWorkRepository.SlotStudentRepository.FirstOrDefaultAsync(sc =>
                sc.SlotId == slotId && sc.UserId == studentId);
        if (slotstudent == null) throw new Exception("Slot Student not found");
    ...
    {
        var slotStudentModel = await _unitOfWorkRepository.SlotStudentRepository.Where(ld => ld.UserId == studentId)
            .ToListAsync();
    ...
    {
        var existSlotStudent =
            await _unitOfWorkRepository.SlotStudentRepository.FirstOrDefaultAsync(ss =>
                ss.SlotId == slotId && ss.UserId == studentId);
    ...
    {
        var slotStudent =
            await _unitOfWorkRepository.SlotStudentRepository.GetSlotStudentBySlotIdAndStudentId(slotId, studentId);

        if (slotStudent == null) throw new NotFoundException("Slot Student not found");
        ...
        {
            Content =
                $"Bạn đã nhận được 1 đánh giá về buổi học {slotStudent.Slot.StartTime} đến {slotStudent.Slot.EndTime} từ học sinh {slotStudent.User.FirstName} {slotStudent.User.LastName}",
    ...
    public async Task CronJobForAutoDereasedMoneyAfterSlotStart()
    {
        ...
        foreach (var slotStudent in slotStudents)
        {
            ...
            var slotCost = (tutor.TutorFeePerHour ?? 0) * (decimal)duration;
            if (studentBalance - slotCost >= 0)
            {
                ...
                {
                    Content =
                        $"Hệ thống đã tự quét trừ {amountToDecrease.ToString("C0", CultureInfo.CreateSpecificCulture("vi-VN"))} từ số dư tài khoản để trả cho Slot học sắp tới của bạn." +
