...
public class TutorVideoService : ITutorVideoService
{
    ...
    public async Task<GetTutorVideoDto> GetTutorVideoByIdAsync(int id)
    {
        ...
        if (tutorVideoEntity == null) throw new NotFoundException($"TutorVideo with ID {id} not found.");
        return tutorVideoEntity.Adapt<GetTutorVideoDto>();
    }

    public async Task<GetTutorVideoDto> CreateTutorVideoAsync(CreateTutorVideoDto tutorVideoDto,
    ...
    public async Task<GetTutorVideoDto> UpdateTutorVideoAsync(UpdateTutorVideoDto tutorVideoDto,
        GetProfileUserDtos user)
    {
        // Retrieve the existing tutor video entity from the database
        var existingTutorVideoEntity =
            await _unitOfWork.TutorVideoRepository.FirstOrDefaultAsync(tv => tv.Id == tutorVideoDto.Id);
    ...
    public async Task<bool> DeleteTutorVideoAsync(int id)
    {
        ...
        if (existingTutorVideoEntity == null) throw new NotFoundException($"TutorVideo with ID {id} not found.");
