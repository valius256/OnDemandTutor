...
public class SlotService : ISlotServices
{
    ...
    {
        if (tutor.Role != RoleStatus.Tutor) throw new BadRequestException("This User is not a tutor");
        ...
        foreach (var studentSlot in listOfStudentSlots)
        {
            ...
            if (slot.StartTime <= studentSlot.Slot.EndTime && slot.EndTime >= studentSlot.Slot.StartTime)
                throw new BadRequestException(
                    $"There is a schedule conflict with slot [Start : {studentSlot.Slot.StartTime}; End : {studentSlot.Slot.EndTime}], please check again");
        }
    }
    ...
    public async Task<GetSlotsDtos> UpdateSlotAsync(UpdateSlotDto slotDto, GetProfileUserDtos user)
    {
        ...
        if (existingSlotEntity == null) throw new NotFoundException($"Slot with ID {slotDto.Id} not found.");
    ...
    public async Task<bool> DeleteSlotAsync(int id)
    {
        ...
        if (!isDeleted) throw new NotFoundException($"Slot with ID {id} not found.");
        return isDeleted;
    }
    ...
    public async Task CronJobForAutoCheckIfStudentDeptIsMoreThan20Percent()
    {
        ...
        foreach (var slotId in slotIds)
        {
            ...
            var notPaidSlotsPercentage = (double)notPaidSlotsCount / totalSlots;

            if (notPaidSlotsPercentage >= 0.20)
                await HandleHighUnpaidSlots(slotId, slotTotalList);
            else if (notPaidSlotsPercentage >= 0.15) await SendPaymentReminder(slotId, slotTotalList);
        }
    }
    ...
    {
        if (slot.StartTime >= slot.EndTime) throw new BadRequestException("Start time must be smaller than end time");
        if (slot.StartTime <= DateTime.Now) throw new BadRequestException("Start time must be in the future");
        var duration = slot.EndTime - slot.StartTime;
        if (duration.TotalMinutes < 15) throw new BadRequestException("Slot duration is minimum 15 minutes");
        if (duration.TotalHours > 4) throw new BadRequestException("Slot duration is maximum 4 hours");
        ...
        foreach (var existSlot in tutorAllSlot)
        {
            ...
            if (slot.StartTime <= existSlot.EndTime && slot.EndTime >= existSlot.StartTime)
                throw new BadRequestException(
                    $"There is a schedule conflict with slot [Start : {existSlot.StartTime}; End : {existSlot.EndTime}], please check again");
        }
    }
        ...
        if (classId.HasValue)
        {
            ...
            catch (Exception ex)
            {
                if (ex.Message.Contains("StudentClass not found"))
                    Console.WriteLine(
                        $"StudentClass not found for user {userDto.Id} in slot {slotId}. Skipping to next.");
                else
                    throw;
            }
        }
