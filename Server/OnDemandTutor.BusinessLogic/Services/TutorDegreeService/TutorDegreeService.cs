...
public class TutorDegreeService : ITutorDegreeService
{
    ...
    public TutorDegreeService(IUnitOfWorkRepository unitOfWorkRepository, IAuthServices authService,
    ...
    {
        var pagedTutorDegrees =
            await _unitOfWorkRepository.TutorDegreeRepository.PagingAsync(request.Adapt<PagingModel<TutorDegree>>());
    ...
    public async Task<GetTutorDegreeDto> GetTutorDegreeByIdAsync(int id)
    {
        ...
        if (tutorDegree == null) throw new NotFoundException($"TutorDegree with ID {id} not found.");
    ...
    {
        var tutorDegree =
            await _unitOfWorkRepository.TutorDegreeRepository.WhereAsync(td =>
                td.TutorId == tutorId && td.SubjectId == subjectId);
    ...
    {
        var tutorDegree = tutorDegreeDto.Adapt<TutorDegree>();
    ...
    public async Task<UpdateTutorDegreeDto> UpdateTutorDegreeAsync(UpdateTutorDegreeDto tutorDegreeDto)
    {
        ...
        var existingTutorDegree =
            await _unitOfWorkRepository.TutorDegreeRepository.FirstOrDefaultAsync(td => td.Id == tutorDegreeDto.Id);
    ...
    {
        var existingTutorDegree =
            await _unitOfWorkRepository.TutorDegreeRepository.FirstOrDefaultAsync(td => td.Id == id);
        if (existingTutorDegree == null) throw new NotFoundException($"TutorDegree with ID {id} not found.");
