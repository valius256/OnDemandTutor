...
public class ConsultationRequestService : IConsultationRequestService
{
    ...
    public ConsultationRequestService(IUnitOfWorkRepository unitOfWorkRepository, IAuthServices authService,
    ...
    public async Task<PagedResult<GetConsultationRequestDto>> GetConsultationRequestsAsync(
        PagingModel<GetConsultationRequestDto> pagingModel)
    {
        var pagedResult =
            await _unitOfWorkRepository.ConsultationRequestRepository.PagingAsync(
                pagingModel.Adapt<PagingModel<Models.Models.ConsultationRequest>>());
    ...
    public async Task<GetConsultationRequestDto> GetConsultationRequestByIdAsync(int id)
    {
        var consultationRequest =
            await _unitOfWorkRepository.ConsultationRequestRepository.FirstOrDefaultAsync(c => c.Id == id);
        return consultationRequest?.Adapt<GetConsultationRequestDto>() ?? throw new InvalidOperationException();
    }

    public async Task<GetConsultationRequestDto> CreateConsultationRequestAsync(
    ...
    public async Task<GetConsultationRequestDto> UpdateConsultationRequestAsync(
        GetConsultationRequestDto consultationRequestDto)
    {
        // Retrieve the existing consultation request entity from the database
        var consultationRequest =
            await _unitOfWorkRepository.ConsultationRequestRepository.FirstOrDefaultAsync(c =>
                c.Id == consultationRequestDto.Id);
        ...
        var updatedConsultationRequestEntity =
    ...
    public async Task<bool> DeleteConsultationRequestAsync(int id)
    {
        var consultationRequest =
            await _unitOfWorkRepository.ConsultationRequestRepository.FirstOrDefaultAsync(c => c.Id == id);
        if (consultationRequest == null) return false;
        _unitOfWorkRepository.ConsultationRequestRepository.Remove(consultationRequest);
        await _unitOfWorkRepository.SaveChangesAsync();
        return true;
    }

    public async Task<bool> HandleConsultationRequestAsync(GetProfileUserDtos user,
        HandleConsultationRequestDto requestDtos)
    {
    ...
    public async Task<PagedResult<GetConsultationRequestDto>> ViewAllConsultationsRequestAsync(
        ConsultationRequestFilterDto request)
    {
