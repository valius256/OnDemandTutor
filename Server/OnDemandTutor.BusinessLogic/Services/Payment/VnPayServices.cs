...
public class VnPayServices : IVnPayServices
{
    ...
    public async Task<string> CreatePaymentForSlotUrl(PaySlotDto model, HttpContext context, GetSlotsDtos slot)
    {
        ...
        var listSlotId = new List<int> { slot.Id };
        ...
        var paymentUrl = CreateVnPayRequest(model, context, new List<int> { slot.Id }, null, slotCost,
            model.OrderDescription, false, tick, model.ReturnUrl);

        var transactionDto = CreateTransactionDto(tick, "Vnpay-bankcode", slotCost, model.OrderDescription, listSlotId,
            null, context, TransactionType.Payment);
    ...
    {
        var tick = DateTime.Now.Ticks.ToString();
        var paymentUrl = CreateVnPayRequest(model, context, null, null, model.Amount, model.Notes, true, tick,
            model.returnUrl);

        var transactionDto = CreateTransactionDto(tick, "Vnpay-bankcode", model.Amount, model.Notes, new List<int>(),
            null, context, TransactionType.Recharge);
        await _transactionServices.CreateTransactionDb(transactionDto);
        return paymentUrl;
    }

    public async Task<string> CreatePaymentForClassUrl(PayClassDto model, HttpContext context,
        GetClassFullDataSlotDto classDto)
    {
        var student = await _authServices.GetUserProfileByClaim(context.User);
        ...
        var totalAmount = (tutorFee * (decimal)totalHoursNotYet)!;
        if (!model.IsFullPay) totalAmount *= 0.2m; // 20% of the total amount

        var tick = DateTime.Now.Ticks.ToString();

        var paymentUrl = CreateVnPayRequest(model, context, slotIds, classDto.Id, totalAmount, model.OrderDescription,
            false, tick, model.returnPage);

        var transactionDto = CreateTransactionDto(tick, "Vnpay-bankcode", totalAmount, model.OrderDescription, slotIds,
            classDto.Id, context);
    ...
    public async Task CreatePaymentForSlotByUserBalance(PaySlotDto model, HttpContext context)
    {
        ...
        if (slotCost > studentBalance) throw new BadRequestException("Inadequate Balance");

        var tick = DateTime.Now.Ticks.ToString();
        var transactionDto = CreateTransactionDto(tick, "User-Balance", slotCost, model.OrderDescription, listSlotId,
            null, context, TransactionType.Payment);
        await _transactionServices.CreateTransactionDb(transactionDto);
    ...
    {
        if (response.SlotId == null) throw new BadRequestException("Payment Error");
    ...
    {
        if (response.ClassId == null) throw new BadRequestException("Payment Error");
        var slotInClass = await _classServices.GetClassByIdAsync(response.ClassId.Value);

        await _transactionServices.CreateTransactionForClassPayment(response.OrderId, response.UserId,
            response.ClassId.Value, response.Money);
    ...
    private PaymentSlotResponseModel CreatePaymentResponseModel(IPaymentResponse response, bool success,
        PaymentStatus paymentStatus, string additionalDescription = "")
    {
    ...
    private string CreateVnPayRequest<T>(T model, HttpContext context, List<int>? slotId, int? classId, decimal amount,
        string? description, bool? isRechargePayment, string tick, string? returnPage)
    {
        ...
        var slotIdString = slotId != null ? string.Join(" ", slotId) : string.Empty;
        ...
        pay.AddRequestData("vnp_OrderInfo",
            $"{isRechargePayment}|{description}|{currUid}|{classId}|{returnPage}|{slotIdString}");
    ...
    private List<TransactionDto> CreateTransactionDto(string tick, string paymentMethod, decimal amount, string? notes,
        List<int> slotIds, int classId, HttpContext context)
    {
        var timeZoneById = TimeZoneInfo.FindSystemTimeZoneById(_configuration["TimeZoneId"]);
        var timeNow = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, timeZoneById);
        var currUid = context.User.FindFirst(c => c.Type == "id")?.Value;
