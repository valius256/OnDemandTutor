...
public class NotificationService : INotificationService
{
    ...
    public async Task<PagedResult<GetNotificationDto>> GetNotificationsAsync(int page, int limit,
        GetProfileUserDtos user)
    {
        var pagedNotifications =
            await _unitOfWork.NotificationRepository.GetNotificationByReceiverId(user.Id, page, limit);
    ...
    public async Task<GetNotificationDto> UpdateViewStatus(int id)
    {
        ...
        if (existingNotification == null) throw new NotFoundException($"Notification with ID {id} not found.");
