...
public class StudentClassService : IStudentClassService
{
    ...
    public StudentClassService(IUnitOfWorkRepository unitOfWork, IAuthServices authService,
        IUserServices userServices, IClassServices classServices, INotificationService notificationService,
    ...
    //}
    public async Task<PagedResult<GetStudentClassDetailDto>> QueryStudentClassAsync(
        PagingModel<QueryStudentClassDto> querySlotStudentDto)
    {
        var slotStudent =
    ...
    public async Task<UpdateStudentClassDto> UpdateStudentClassAsync(UpdateStudentClassDto studentClassDto)
    {
        ...
        var existingStudentClassEntity =
            await _unitOfWork.StudentClassRepository.FirstOrDefaultAsync(sc => sc.Id == studentClassDto.Id);
    ...
    {
        var studentClass = await _unitOfWork.StudentClassRepository.FirstOrDefaultAsync(sc => sc.Id == id);
        if (studentClass == null) throw new Exception("StudentClass not found");
    ...
    {
        var recordInDB =
            await _unitOfWork.StudentClassRepository.FirstOrDefaultAsync(st =>
                st.StudentId == studentId && st.ClassId == classId);
        if (recordInDB == null) throw new ModelException($"{recordInDB.Id}", "has not found");
    ...
    {
        var studentClass =
            await _unitOfWork.StudentClassRepository.FirstOrDefaultAsync(sc =>
                sc.ClassId == classId && sc.StudentId == userId);
        if (studentClass == null) throw new Exception("StudentClass not found");
        _unitOfWork.StudentClassRepository.Remove(studentClass);
