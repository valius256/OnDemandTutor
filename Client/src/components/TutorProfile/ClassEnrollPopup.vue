  ...
  <div class="p-4 bg-white rounded-b-lg w-full" v-if="this.class">
    <div class="flex flex-col w-96">
      ...
      <div class="mt-4 italic text-center">
        Theo quy định, bạn sẽ phải cọc trước 20% tổng thời lượng các slot nhân
        với giá dịch vụ theo giờ của gia sư.
      </div>
    ...
    <div class="flex flex-col w-96 mt-4">
      <button
        class="mt-8 font-bold text-white bg-blue-400 hover:bg-blue-200 rounded-lg py-2"
        @click="handleConfirm"
      >
        Xác nhận
      </button>
  ...
  methods: {
    calcMoney() {
      ...
      for (var slot of this.class.slots) {
        money +=
          (this.calcDuration(slot) * this.class.tutor.tutorFeePerHour * 20) /
          100;
      }
      return money;
    },
      ...
      if (confirmation) {
        ...
        try {
          const url = await axios.post(
            import.meta.env.VITE_API_URL + "/api/Payment/create-payment-class",
            request,
            {
              headers: {
                Authorization: "Bearer " + localStorage.token,
