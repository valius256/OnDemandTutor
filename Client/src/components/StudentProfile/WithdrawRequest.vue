<template>
    <div>
        ...
        <div class="px-4 mb-4">
            <table class="bg-slate-50 p-6 rounded-xl text-center w-full" v-if="withdrawRequests.length > 0">
                <thead class="mt-2">
                    <th>Số lượng</th>
                    <th>Lý do</th>
                ...
                <tbody>
                    <tr v-for="request in withdrawRequests" :key="request.id" class="border-2 border-white">
                        <td class="p-2">
                            {{ request.amount.toLocaleString('vi-VN', {style: 'currency',currency: 'VND',})}}
                        </td>
                        ...
                        <td class="break-words w-32">{{ this.beautifyDatetime(request.createdDate) }}</td>
                        <td>{{ request.operator?.name}}</td>
                        <td class="break-words w-32">{{ request.reply }}</td>
                        <td class="break-words w-32">{{ this.beautifyDatetime(request.updatedDate) }}</td>
                        <td>
            ...
            <div class="flex gap-4 justify-center mt-4" v-if="withdrawRequests.length > 0">
...
export default {
    ...
    methods: {
        async fetchWithdraw() {
            ...
            const response = await axios.get(import.meta.env.VITE_API_URL + '/api/RequestWithDraw/all?' +
